...
std::ifstream file("test2.txt"); //test line
bool switcher{true};
...
int Primary() {
    ...
    if(current_word == "(") {
        ...
        if(Expression()) {
            if(current_word == ")") {
                std::cout << "done correct WAAAAAAAAAAA\n";
                printToken(token, current_word);
                moveFile();
                return 1;
            }
